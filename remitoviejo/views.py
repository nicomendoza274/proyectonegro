...
def index3(request):
    ...
    ins = Insumos.objects.filter(estado=True)
...
def validarremito(request):
    try:
        response_data = {}
        insumo = Insumos.objects.get(codigo=request.GET['codigo_abuscar'], estado=True)
        print(insumo.nombre)
...
def editarcabecera(request):
    try:
        fecremit=datetime.datetime.strptime(request.POST['fecven2'],'%d/%m/%Y').strftime('%Y-%m-%d')
        prov=Proveedores.objects.get(pk=request.POST['nombprov2'])
        cabecer = Copia_Remitos.objects.get(pk=request.POST['editarcabecera'])
        cabecer.prov_id2= prov
        cabecer.fecha_recib2= fecremit
        cabecer.codigo2 = request.POST['codigo2']
...
def editardetalle(request):
    try:
        ...
        depos= Depositos.objects.get(pk=request.POST['numdep'])
        detalle = Copia_DetRem.objects.get(id=request.POST['editardetalle'])
        detalle.cant_recib2 = request.POST['canti']
        detalle.vencimiento2 = fecven
        detalle.dep_id2_id = depos.id
    except(KeyError):
        return HttpResponseRedirect(reverse("remito:index2"))
    else:
        detalle.save()
...
def guardarcabecera(request):
    codigopiola=request.POST['codrem']
    codcab=Remitos.objects.filter(codigo=codigopiola)
    if not codcab:
        ...
        prov = Proveedores.objects.filter(estado=True)
        rem = Remitos.objects.all()
        error_remitoExistente = 'Codigo de remito existente'
        return render(request, "remito/cabecera.html" , {'error_remitoExistente': error_remitoExistente, 'det': det, 'ins': ins, 'dep': dep, 'rem': rem, 'cont': cont, 'prov': prov})
