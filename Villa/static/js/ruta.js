...
var HQ =  new google.maps.LatLng(-24.772033, -65.421870);
var coordenadas;
...
function agregarMarcadorArray(latitud,longitud){
    // Add a node to map
    /*if (latitud != ''){
        coordenadas = new google.maps.LatLng(latitud,longitud);
        marker = new google.maps.Marker({position: coordenadas, map: map});
        markers.push(marker);
        // Store node's lat and lng
        nodes.push(coordenadas);
        }*/
   if (latitud != ''){
    coordenadas = new google.maps.LatLng(latitud,longitud);
    nodes.push(coordenadas);
